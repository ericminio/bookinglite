...
Calendar.prototype.buildMonthRow = function(){
  ...
  return cheerio('<tr class=month><td></td> <td colspan'+daysInMonth+'>'+dateUtils.getMonth(this.date)+'</td> </tr>');
};
...
Calendar.prototype.buildDayWeekRow = function(calendarSection){
  ...
  for(i=0;i<daysInMonth;i++){
    ...
    var newTd = cheerio('<td class=weekday'+currentDate.getDay()+'>'+dateUtils.getShortWeekDay(this.date)+'</td>');
    newTr.append(newTd);
  }
